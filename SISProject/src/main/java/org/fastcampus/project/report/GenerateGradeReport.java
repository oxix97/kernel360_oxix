...
public class GenerateGradeReport {
    ...
    public void makeBody(SubjectJ subjectJ) {
        ...
        ArrayList<StudentJ> studentJList = subjectJ.getStudentJList();  // 각 과목의 학생들
    ...
    public void evaluateGradeByScoreAndStrategy(SubjectJ subjectJ, ScoreJ score, int majorId) {
        ...
        if (subjectJ.getGradeType() == Define.PF_TYPE.code) {
            grade = gradeEvaluation[Define.PF_TYPE.code].getGrade(score.getPoint());
        } else {
            if (score.getSubject().getSubjectId() == majorId)  // 중점 과목인 경우
                grade = gradeEvaluation[Define.SAB_TYPE.code].getGrade(score.getPoint());  //중점 과목 학점 평가 방법
            else
                grade = gradeEvaluation[Define.AB_TYPE.code].getGrade(score.getPoint()); // 중점 과목이 아닌 경우
        }
    ...
    public static String evaluateGradeByScoreAndStrategy(ScoreDto score) {
        SubjectDto subject = score.subject();
        StudentDto student = score.student();
        GradeEvaluation[] gradeEvaluation = {new BasicEvaluation(), new MajorEvaluation(), new PassFailEvaluation()};
        String grade;

        if (subject.gradeType() == Define.PF_TYPE.code) {
            grade = gradeEvaluation[Define.PF_TYPE.code].getGrade(score.value());
        } else {
            if (subject.id() == student.majorSubject())  // 중점 과목인 경우
                grade = gradeEvaluation[Define.SAB_TYPE.code].getGrade(score.value());  //중점 과목 학점 평가 방법
            else
                grade = gradeEvaluation[Define.AB_TYPE.code].getGrade(score.value()); // 중점 과목이 아닌 경우
        }
        return grade;
    }
